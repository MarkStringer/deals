...
df['Create Date'] = pd.to_datetime(df['Create Date'], format='%Y-%m-%d %H:%M')
# sort the DataFrame by the Create Date column
df = df.sort_values('Create Date')

# Get all unique stages that start with "Closed"
closed_stages = df['Deal Stage'][df['Deal Stage'].str.startswith('Closed', na=False)].unique()
...
not_closed_mask = ~df['Deal Stage'].str.startswith('Closed', na=False)
